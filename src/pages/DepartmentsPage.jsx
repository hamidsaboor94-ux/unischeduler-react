...
import { deleteDepartment, saveCollege, deleteCollege } from '../api.js';

export default function DepartmentsPage() {
  const { t } = useTranslation(['management', 'common']);
  const { departments, colleges, courses, teachers, afterMutate } = useAppData();
  ...
  return (
    <Section name="departments">
      ...
      <div id="content">
        {/* Colleges: the optional layer above departments that a Dean is scoped to. */}
              ...
              {departments.map(d => {
                ...
                return (
                  <tr key={d.id} onClick={() => openModal('department', d.id)}>
                    <td>{d.name}</td>
                    <td>{collegeName(d.collegeId) || <span style={{ color: 'var(--text-muted)' }}>{t('common:notApplicable')}</span>}</td>
